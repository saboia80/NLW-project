...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
  const [shouldShowOnBoarding, setShouldShowOnBoearding] = useState(true);
  const [isRecording, setIsRecording] = useState(false);
  const [content, setContent] = useState("");

  function handleStartEditor() {
    setShouldShowOnBoearding(false);
  }

  function handleContenChanged(event: ChangeEvent<HTMLTextAreaElement>) {
    setContent(event.target.value);

    if (event.target.value === "") {
      setShouldShowOnBoearding(true);
    }
  }
  ...
  function handleSaveNote(event: FormEvent) {
    ...
    setContent("");
    setShouldShowOnBoearding(true);
    toast.success("nota craiada com sucesso!");
  }
          ...
          <form className="flex-1 flex flex-col">
            ...
            {isRecording ? (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-slate-900 py-4 text-center text-small text-slate-300 font-medium hover:text-slate-100"
              >
              ...
              <button
                ...
                className="w-full bg-lime-400 py-4 text-center text-small text-lime-950 font-medium hover:bg-lime-500"
              >
                Salvar nota
              </button>
            )}
